...
def generate_json_for_datagrid(obj, success=True, error_message = "Saved Successfully", form_errors  = None):
    ...
    try:
      ...
      # set a basic attribute dictionary and getattr method specifying default fallbacks
      iterable = iter(obj) 
      if iterable:
        for element in obj:
            ...
            if i.name not in data.keys():
              ...
              try:
                if i.name == 'aushadhabasemodel_ptr':
                  ...
                  data[i.name] = getattr(element, '__unicode__()',element.administrator.__unicode__())
                elif i.name=="vaccine_detail":
                  data[i.name] = getattr(element, '__unicode__()',element.vaccine_detail.vaccine_name)
                else:
        ...
        if i.name not in data.keys():
          ...
          try:
            if i.name == 'aushadhabasemodel_ptr':
              ...
              data[i.name] = getattr(obj, 'username', obj.administrator.username)
            elif i.name=="vaccine_detail":
              data[i.name] = getattr(obj, 'vaccine_name',obj.vaccine_detail.vaccine_name)
            else:
  ...
  if request.user:
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        ...
        variable                  = RequestContext(request, {"user" 									:	user,
                                                              "patient_detail_obj"			:	patient_detail_obj ,
                                                              "patient_contact_add_form" 	:	patient_contact_add_form, 
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        ...
        variable                  = RequestContext(request, 
        																						{"user" :	user,
        																						 "patient_detail_obj"			  :	patient_detail_obj ,
        																						 "patient_contact_edit_form":	patient_contact_edit_form, 
																										 "patient_contact_obj" 			:	patient_contact_obj ,
																									})
      except TypeError or ValueError or AttributeError:
        raise Http404("BadRequest")
  ...
  if request.user:
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        ...
        variable                = RequestContext(request, {"user" 									:	user,
                                                            "patient_detail_obj"			:	patient_detail_obj ,
                                                            "patient_email_and_fax_add_form" 	:	patient_email_and_fax_add_form, 
  ...
  if request.user:
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        ...
        variable                = RequestContext(request, {"user" :	user,
        																										"patient_detail_obj"			:	patient_detail_obj ,
        																										"patient_email_and_fax_edit_form"	:	patient_email_and_fax_edit_form, 
																														"patient_email_and_fax_obj" 			:	patient_email_and_fax_obj ,
																									})
      except TypeError or ValueError or AttributeError:
        raise Http404("BadRequest")
